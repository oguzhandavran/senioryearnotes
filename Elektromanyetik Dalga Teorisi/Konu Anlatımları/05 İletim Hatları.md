...
```mermaid
flowchart TD
    SM["<b>Smith Çizimi</b><br/>(Normalize empedans)"]
    SM --> R["r = sabit daireler<br/><i>Direnç çemberleri</i>"]
    SM --> X["x = sabit yaylar<br/><i>Reaktans eğrileri</i>"]
    R --> Rc["Merkez: Γ = 0<br/><i>Z = Z₀ (uyumlu)</i>"]
    X --> Xc["Dış halka: |Γ| = 1<br/>Tam yansıma"]
    style SM fill:#1a1a2e,color:#ffffff,stroke:#1a1a2e
    style R fill:#eef2f7,stroke:#1a1a2e
    style X fill:#eef2f7,stroke:#1a1a2e
    style Rc fill:#d6e0f0,stroke:#1a1a2e
    style Xc fill:#d6e0f0,stroke:#1a1a2e
```

*Saat yönü → yüke doğru (WTL)  ·  Tersi → generatöre (WTG)  ·  λ/4 = 180°*
