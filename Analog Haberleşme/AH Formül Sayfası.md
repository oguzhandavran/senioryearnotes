...
```mermaid
flowchart TD
    Q{"İşaret periyodik mi?"}
    Q -->|"Evet"| FS["<b>Fourier Serisi</b><br/>cₙ katsayılarını hesapla<br/><i>X(f) = Σ cₙ δ(f − n/T₀)</i>"]
    Q -->|"Hayır"| FT["<b>Fourier Dönüşümü</b><br/>FT tablosu + özellikler"]
    FT --> TS["Zaman kaydırma?<br/><i>× e^{−j2πft₀}</i>"]
    FT --> MD["Modülasyon?<br/><i>X(f ± fc) / 2</i>"]
    FS --> R["<b>Genlik/Faz spektrumu çiz ✓</b><br/>|X(f)| ve ∠X(f) grafiğini çiz"]
    TS --> R
    MD --> R
    style Q fill:#eef2f7,stroke:#1a1a2e
    style FS fill:#d6e0f0,stroke:#1a1a2e
    style FT fill:#eef2f7,stroke:#1a1a2e
    style TS fill:#eef2f7,stroke:#1a1a2e
    style MD fill:#eef2f7,stroke:#1a1a2e
