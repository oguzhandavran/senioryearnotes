...
> [!info] Semboller
> - **$m$**: kütle, **$l$**: çubuk uzunluğu, **$g$**: yerçekimi ivmesi
> - **$\theta$**: sarkacın açısı (durum), **$b$**: sönüm (sürtünme) katsayısı
> - **$\tau$**: dışarıdan uygulanan **tork** → sistemin **girişi** ($u = \tau$)
>
> Terimlerin anlamı:
> - $ml^2\ddot{\theta}$ → eylemsizlik (atalet)
> - $b\dot{\theta}$ → sönüm (sürtünme)
> - $mgl\sin\theta$ → yerçekiminin geri çağırıcı etkisi
> - $\tau$ → seni hareket ettiren giriş kuvveti
...
**2a) İlk denklem (tanımdan gelir):**

$x_1 = \theta$ seçtik, türevini al → $\dot{x}_1 = \dot{\theta}$. Ama $\dot{\theta} = x_2$ idi. Demek ki:
$$\dot{x}_1 = x_2$$

**2b) İkinci denklem — yerine koyma:**

$x_2 = \dot{\theta}$ olduğundan türevi $\dot{x}_2 = \ddot{\theta}$'dır. Orijinal denklemde $\theta \to x_1$, $\dot{\theta} \to x_2$, $\ddot{\theta} \to \dot{x}_2$ koy:
$$ml^2\,\dot{x}_2 + b\,x_2 + mgl\sin x_1 = \tau$$

**2c) $\dot{x}_2$'yi yalnız bırak** (diğer terimleri sağa at, $ml^2$'ye böl):
$$ml^2\,\dot{x}_2 = \tau - b\,x_2 - mgl\sin x_1$$

$$\boxed{\dot{x}_2 = \frac{\tau}{ml^2} - \frac{b}{ml^2}x_2 - \frac{g}{l}\sin x_1}$$

*(son terimde $\dfrac{mgl}{ml^2} = \dfrac{g}{l}$ sadeleşti)*
